...
use plonky2::plonk::config::CpuProverCompute;
...
use crate::fr_bn254::FrBn254Repr;
use crate::poseidon_bls12_381::permution;
use crate::poseidon_bls12_381::GOLDILOCKS_ELEMENTS;
use crate::poseidon_bls12_381::RATE;
...
fn bn254_hash_out_to_bytes<F: Field>(hash: PoseidonBN254HashOut<F>) -> Vec<u8> {
...
impl<F: RichField> GenericHashOut<F> for PoseidonBN254HashOut<F> {
    fn to_bytes(&self) -> Vec<u8> {
        bn254_hash_out_to_bytes(*self)
    }

    fn from_bytes(bytes: &[u8]) -> Self {
        let sized_bytes: [u8; 32] = bytes.try_into().unwrap();
        let fr_repr = FrBn254Repr(sized_bytes);
        let fr = FrBn254::from_repr(fr_repr).unwrap();
        Self {
            value: fr,
            _phantom: PhantomData,
        }
    ...
    fn to_vec(&self) -> Vec<F> {
        let bytes = bn254_hash_out_to_bytes(*self);
        bytes
            .chunks(7)
            .map(|bytes| {
                let mut arr = [0; 8];
...
impl<F: RichField> Hasher<F> for PoseidonBN254Hash {
    ...
    fn hash_no_pad(input: &[F]) -> Self::Hash {
        let mut state = [FrBn254::ZERO; poseidon_bn254::WIDTH];

        for rate_chunk in input.chunks(poseidon_bn254::RATE * poseidon_bn254::GOLDILOCKS_ELEMENTS) {
            for (j, bn254_chunk) in rate_chunk
                .chunks(poseidon_bn254::GOLDILOCKS_ELEMENTS)
                .enumerate()
            {
                ...
                let sized_bytes: [u8; 32] = bytes.try_into().unwrap();
                let fr_repr = FrBn254Repr(sized_bytes);
                state[j + 1] = FrBn254::from_repr(fr_repr).unwrap();
            }
            poseidon_bn254::permution(&mut state);
        }
    ...
    fn hash_pad(input: &[F]) -> Self::Hash {
        ...
        while (padded_input.len() + 1)
            % (poseidon_bn254::RATE * poseidon_bn254::GOLDILOCKS_ELEMENTS)
            != 0
        {
            padded_input.push(F::ZERO);
        }
    ...
    fn hash_or_noop(inputs: &[F]) -> Self::Hash {
        if inputs.len() * 8 <= poseidon_bn254::GOLDILOCKS_ELEMENTS * 8 {
            let mut inputs_bytes = vec![0u8; 32];
            for i in 0..inputs.len() {
                inputs_bytes[i * 8..(i + 1) * 8]
                    .copy_from_slice(&inputs[i].to_canonical_u64().to_le_bytes());
    ...
    fn two_to_one(left: Self::Hash, right: Self::Hash) -> Self::Hash {
        let mut state = [FrBn254::ZERO, FrBn254::ZERO, left.value, right.value];
        poseidon_bn254::permution(&mut state);
        PoseidonBN254HashOut {
            value: state[0],
            _phantom: PhantomData,
